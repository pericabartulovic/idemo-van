...
class Setnja extends Aktivnost {
    type = 'setnja';
    constructor(coords1, distance1, duration1, note1){
        super(coords1, distance1, duration1, note1);
        this._setDescription();
    }
}
class Bicikliranje extends Aktivnost {
    type = 'bicikliranje';
    constructor(coords2, distance2, duration2, note2){
        super(coords2, distance2, duration2, note2);
        this._setDescription();
    }
}
class Rolanje extends Aktivnost {
    type = 'romobil';
    constructor(coords3, distance3, duration3, note3){
        super(coords3, distance3, duration3, note3);
        this._setDescription();
    }
}
class Izlet extends Aktivnost {
    type = 'izlet';
    constructor(coords4, distance4, duration4, note4){
        super(coords4, distance4, duration4, note4);
        this._setDescription();
    }
}
class Sanjkanje extends Aktivnost {
    type = 'sanjkanje';
    constructor(coords5, distance5, duration5, note5){
        super(coords5, distance5, duration5, note5);
        this._setDescription();
    }
}
///////////////////////////////////////////////////////////// APP ARCHITECTURE  ///////////////////////////////////////////////////////////
...
class App {
    ...
    constructor(){
        ...
        Array.from(godisnjaDoba).forEach((el)=>el.addEventListener('click', this._prikazAktivKarta.bind(this))
        );
        Array.from(holder).forEach((el)=>el.addEventListener('click', this._prikazLista.bind(this))
        );
    ...
    _loadMap(position) {
        ...
        }), ulice = L.tileLayer('https://{s}.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png', {
    ...
    inputCadence.closest('.form__row').classList.toggle('form__row--hidden');
        ...
        if (type === 'setnja') {
            ...
            if (!isValid(distance, duration)) return alert('Unos mora biti pozitivan broj!');
            aktivnost = new Setnja([
                lat,
                lng
            ], distance, duration, note);
        }
        if (type === 'bicikliranje') {
            if (!isValid(distance, duration)) return alert('Unos mora biti pozitivan broj!');
            aktivnost = new Bicikliranje([
                lat,
                lng
            ], distance, duration, note);
        }
    ...
    _renderAktivnostMarker(aktiv1) {
        let godDoba = this._getGodDoba(aktiv1);
        let aktivnost;
        aktiv1.type === 'setnja' ? aktivnost = '🚶‍♀️🚶‍♂️🚶‍♀️' : aktiv1.type;
        aktiv1.type === 'bicikliranje' ? aktivnost = '🚲🚲🚲' : aktiv1.type;
        aktiv1.type === 'romobil' ? aktivnost = '🛴' : aktiv1.type;
        aktiv1.type === 'izlet' ? aktivnost = '🗻' : aktiv1.type;
        aktiv1.type === 'sanjkanje' ? aktivnost = '❄❄❄ ' : aktiv1.type;
        L.marker(aktiv1.coords).addTo(this.#map).bindPopup(L.popup({
            maxWidth: 250,
            minWidth: 100,
            autoClose: false,
            closeOnClick: false,
            className: `${godDoba}-popup`
        })).setPopupContent(`${aktivnost} ${aktiv1.description}`).openPopup();
    }
    ...
    _renderAktivnost(aktivnost1) {
        let aktiv;
        aktivnost1.type === 'setnja' ? aktiv = '🚶‍♀️🚶‍♂️🚶‍♀️' : aktivnost1.type;
        ...
        aktivnost1.duration === 3 || aktivnost1.duration === 23 || aktivnost1.duration === 33 ? sat = 'sata' : aktivnost1.duration;
        aktivnost1.duration === 4 || aktivnost1.duration === 24 || aktivnost1.duration === 34 ? sat = 'sata' : aktivnost1.duration;
        const doba = this._getGodDoba(aktivnost1);
        //prettier-ignore
        let html = `
    <li class="aktivnost aktivnost--${aktivnost1.type} aktivnost--${doba}" data-id="${aktivnost1.id}">
      <h2 class="aktivnost__title">${aktivnost1.description}</h2>
        <div class="aktivnost__details ikone" style="grid-column: 2">
          <span class="aktivnost__icon"> ${aktiv} </span>
          <span class="aktivnost__value">${aktivnost1.distance}</span>
          <span class="aktivnost__unit">km</span>
        </div>
        <div class="aktivnost__details" style="grid-column: 3">
          <span class="aktivnost__icon">⏱</span>
          <span class="aktivnost__value">${aktivnost1.duration}</span>
          <span class="aktivnost__unit">${sat}</span>
        </div>
        <div class="aktivnost__details aktivnost__details-note">
          <span class="aktivnost__icon">📝</span>
          <span class="aktivnost__value biljeska">${aktivnost1.note}</span>
        </div>
    </li>
            `;
        const pljjz = [
        ...
        pljjz.forEach((dob)=>{
            if (dob.classList.contains(doba) && doba === 'proljece') proljeceLista.insertAdjacentHTML('afterbegin', html);
            if (dob.classList.contains(doba) && doba === 'ljeto') ljetoLista.insertAdjacentHTML('afterbegin', html);
            if (dob.classList.contains(doba) && doba === 'jesen') jesenLista.insertAdjacentHTML('afterbegin', html);
            if (dob.classList.contains(doba) && doba === 'zima') zimaLista.insertAdjacentHTML('afterbegin', html);
        });
    ...
    _prikazAktivKarta(e1) {
        if (!this.#map) return;
        //const [lat, lng] = this.coords;
        const aktivnostEl = e1.target.closest('.aktivnost');
        ...
        Array.from(aktivnostiSve).forEach((a)=>a.classList.remove('aktivnost--aktivna')
        );
        aktivnost.id === aktivnostEl.dataset.id && aktivnostEl.classList.add('aktivnost--aktivna');
        /////////////////////animirani prijelazi ////////////
        // prettier-ignore
        this.#map.flyTo(aktivnost.coords, 16, {
            animate: true,
            duration: 1
        });
        setTimeout(()=>this.#map.flyTo(aktivnost.coords, 15, {
                animate: true,
                duration: 1
            })
        , 2000);
        ...
        if (window.matchMedia("(max-width: 48em").matches) {
            prsten.classList.contains('map-prsten') && prsten.classList.remove('map-prsten');
            mapa.classList.contains('map-prsten') && mapa.classList.remove('map-prsten');
        }
    }
    _prikazLista(e2) {
